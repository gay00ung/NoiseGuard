...
class NoiseClassifier(
    ...
    private var classificationExecutor: java.util.concurrent.ScheduledExecutorService? = null
    @Volatile private var isClassifying: Boolean = false
    ...
    override fun startRecordingAndClassifying(onResult: (List<ClassifiedLabel>) -> Unit) {
        ...
        if (!this::classifier.isInitialized) {
            Log.e("NoiseClassifier", "Classifier is not initialized. Call initialize() first.")
            return
        }

        if (isClassifying) {
            Log.w("NoiseClassifier", "Classifier already running; restarting")
            stopRecording()
        }

        if (this::audioRecord.isInitialized && audioRecord.state == AudioRecord.STATE_INITIALIZED) {
        ...
        try {
            ...
            Log.e("NoiseClassifier", "Failed to start AudioRecord", e)
            return
        }
        ...
        classificationExecutor?.scheduleWithFixedDelay({
            if (!isClassifying) {
            ...
            try {
                ...
                val read = audioRecord.read(buf, 0, len, AudioRecord.READ_BLOCKING)
                if (read <= 0) {
                    Log.e("NoiseClassifier", "AudioRecord read failed with code $read")
                    handleAudioFailure()
                    return@scheduleWithFixedDelay
                }
                audioData.load(buf)
                ...
                Log.e("NoiseClassifier", "Classification loop crashed", t)
                handleAudioFailure()
            }
